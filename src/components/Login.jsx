import axios from "axios";
import { useState } from "react";
import { useDispatch } from "react-redux";
import { Navigate, useNavigate } from "react-router-dom";
import { addUser } from "../utils/userSlice";
import { API_DOMAIN_URL } from "../utils/constants";

const Login = () => {
    const [userName, setUserName] = useState('');
    const [password, setPassword] = useState('');
    const [error, setError] = useState('');

    const navigate = useNavigate();
    const dispatch = useDispatch();

    const onLoginClick = async () => {
        try {
            const userData = await axios.post(`${API_DOMAIN_URL}/login`, {
                "emailId": userName,
                "password": password
            }, { withCredentials: true });
            console.log(userData.data);
            dispatch(addUser(userData.data));
            navigate('/feed');
        } catch (err) {
            setError(err.response.data)
        }
    }

    return (
        <div className="card bg-primary text-primary-content w-96 justify-self-center mt-36 h-96">
            <div className="card-body">
                <h2 className="card-title">Login</h2>
                <label className="input input-bordered flex items-center gap-2 mt-4 bg-base-100">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 16 16"
                        fill="currentColor"
                        className="h-4 w-4 opacity-70">
                        <path
                            d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM12.735 14c.618 0 1.093-.561.872-1.139a6.002 6.002 0 0 0-11.215 0c-.22.578.254 1.139.872 1.139h9.47Z" />
                    </svg>
                    <input type="text" className="grow" placeholder="Username" onChange={(e) => setUserName(e.target.value)} />
                </label>
                <label className="input input-bordered flex items-center gap-2 mt-4 bg-base-100 ">
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 16 16"
                        fill="currentColor"
                        className="h-4 w-4 opacity-70">
                        <path
                            fillRule="evenodd"
                            d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z"
                            clipRule="evenodd" />
                    </svg>
                    <input type="password" placeholder="password" className="grow" onChange={(e) => setPassword(e.target.value)} />
                </label>
                {error && <div><span className="text-error"> {error}</span></div>}
                <div className="card-actions mt-8 justify-center">
                    <button className="btn w-32" onClick={onLoginClick}>Login</button>
                </div>
            </div>
        </div>
    )
}

export default Login;
